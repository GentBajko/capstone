...
Resume = read the file, never re-ask.

Prerequisite: `mockup-interview.md` is `formalized` and
`docs/capstone/mockup/` exists — the screens are this stage's unit of
work. Otherwise say so, point at `mockup` (or `start`), and stop.
