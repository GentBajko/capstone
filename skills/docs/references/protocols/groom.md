...
2. From the DESIGN.md index, pick the topics the feature touches —
   ...
   implies. Run the `check-docs` staleness test on just those topics
...
3. Read the picked chapters, plus whichever companion docs bear on the
   ...
   it changes, `code-prefs.md`. Never ask what these already answer.
...
- **Reference impact** — which chapters and scenarios this will change
  once built (`plan` reads this to scope its study; the `docs` refresh
  and `changelog` settle it after `implement`).
