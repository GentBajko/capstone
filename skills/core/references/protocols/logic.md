...
seeds the interview per core.md's Artifact seeding rule. Build the scenario list:
...
**Extraction mode (brownfield):** when there is no mockup but the repo
has code and a stamped reference, offer to extract instead of elicit:
read the chapters (`04-data-flow.md`, `02-models.md`,
`01-architecture.md`; source only where a chapter labels its coverage
shallow), draft the scenario list from the observed flows, and confirm
it with the user like any elicited list. Then per scenario, draft the
file from observed behavior — every rule cited `file:line` — present
the draft, and only the user's confirmation or correction makes it
normative; corrections are recorded as `### Q<n>` entries like any
answer. Extraction outputs carry `paths_covered` (the globs the
scenario's rules were read from), so `sync` refreshes them as code
moves; interview-derived files stay stamp-only and are absorbed by
`implement` instead.
